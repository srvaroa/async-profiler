...
class Recording {
  ...
  public:
    ...
    u64 classId(const char* class_name, size_t length) {
        return (uintptr_t) Profiler::_instance.classMap()->lookup(class_name, length);
    }

    u64 symbolId(const char* symbol_name, size_t length) {
        return (uintptr_t) Profiler::_instance.symbolMap()->lookup(symbol_name, length);
    }

    u64 classId(const char* class_name) {
        return classId(class_name, strlen(class_name));
    }

    u64 symbolId(const char* symbol_name) {
    ...
    void writeClasses(Buffer* buf) {
        ...
        for (int i = 0; i < classes.size(); i++) {
            buf->put64((uintptr_t)classes[i]);
            buf->put64(0);  // loader class
            buf->put64(symbolId(classes[i]));
    ...
    void writeSymbols(Buffer* buf) {
        ...
        for (int i = 0; i < symbols.size(); i++) {
            buf->put64((uintptr_t)symbols[i]);
            buf->putUtf8(symbols[i]);
            flushIfNeeded(buf);
        }
    }
    ...
    void recordExecutionSample(Buffer* buf, int tid, int call_trace_id, ExecutionEvent* event) {
        buf->put32(30);
    ...
    void recordAllocationInNewTLAB(Buffer* buf, int tid, int call_trace_id, AllocEvent* event) {
        buf->put32(52);
        buf->put32(EVENT_ALLOC_IN_NEW_TLAB);
        buf->put64(OS::nanotime());
        buf->put32(tid);
        buf->put64(call_trace_id);
        buf->put64((uintptr_t)event->_class_name);
        buf->put64(event->_instance_size);
        buf->put64(event->_total_size);
    }

    void recordAllocationOutsideTLAB(Buffer* buf, int tid, int call_trace_id, AllocEvent* event) {
        buf->put32(44);
    ...
    void recordMonitorBlocked(Buffer* buf, int tid, int call_trace_id, LockEvent* event) {
        buf->put32(56);
    ...
    void recordThreadPark(Buffer* buf, int tid, int call_trace_id, LockEvent* event) {
        buf->put32(60);
...
void FlightRecorder::recordEvent(int lock_index, int tid, int call_trace_id,
